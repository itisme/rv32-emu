...
struct riscv_internal {
    volatile bool halt; /* indicate whether the core is halted */
    ...
    /* Global variables */
    uint32_t last_pc; /* last pc for block chaining */
    bool is_branch_taken; /* whether the branch is taken or not */
    block_t *prev; /* previous block for block chaining */
...
#if RV32_HAS(EXT_TT)
FORCE_INLINE uint32_t rotr32(uint32_t x, unsigned k) {
    return (x >> k) | (x << (32 - k));
}

FORCE_INLINE uint32_t rotl32(uint32_t x, unsigned k) {
    return (x << k) | (x >> (32 - k));
}
#endif

FORCE_INLINE bool is_tt(uint32_t insn)
{
#if RV32_HAS(EXT_TT)
