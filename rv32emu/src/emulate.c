...
static bool is_wait(uint8_t opcode)
{
    return opcode == 0xa6 || opcode == 0xa7;
}

/* Execute a Tensix instruction (inline TT instruction in code)
 *
 * Uses direct instruction interface (not FIFO) to support proper blocking
 * on Wait Gate. The instruction is stored in ir->imm during block translation.
 *
 * Returns:
 *   true  - instruction executed successfully, PC should advance
 *   false - instruction blocked by Wait Gate, PC stays same
 */
static bool do_ttinsn(riscv_t *rv, const rv_insn_t *ir,
                      uint64_t cycle, uint32_t PC)
{
    cycle++;  /* Hardware clock always ticks, even when blocked */

    uint32_t insn = (uint32_t)ir->imm;

    /* Set direct instruction (not FIFO) */
    tensix_set_direct(rv->tensix, insn, rv->core_id);

    /* Execute direct instruction */
    if (tensix_step_direct(rv->tensix, rv->core_id)) {
        /* Instruction executed (or Wait instruction latched to Wait Gate) */
        rv->csr_cycle = cycle;
        rv->PC = PC + 4;
        return true;
    }

    /* Blocked by Wait Gate, don't advance PC */
    rv->csr_cycle = cycle;
    rv->PC = PC;
    return false;
}
...
#if RV32_HAS(EXT_TT)
            /* This is a Tensix instruction, not RISC-V */
            insn = rotr32(insn, 2);

            /* Setup IR for this TT instruction.
             * Store full instruction in ir->imm for push during execution.
             * This ensures correct behavior with block caching.
             */
            ir->opcode = rv_insn_ttinsn;
            ir->imm = (int32_t)insn;  /* Store full 32-bit TT instruction */
            ir->impl = do_ttinsn;
            ir->pc = block->pc_end;
            block->pc_end += 4;
            block->n_insn++;
...
#endif
            /* Stop here: one TT instruction per block for now (easier debugging) */
            break;
        }
...
void rv_step(void *arg)
{
    ...
    while (rv->csr_cycle < cycles_target && !rv->halt) {
        ...
        if (rv->prev && rv->prev->pc_start != rv->last_pc) {
            /* update previous block */
#if !RV32_HAS(JIT)
            rv->prev = block_find(&rv->block_map, rv->last_pc);
#else
        ...
        if (rv->prev
#if RV32_HAS(JIT) && RV32_HAS(SYSTEM)
            && rv->prev->satp == rv->csr_satp
#endif
        ) {
            rv_insn_t *last_ir = rv->prev->ir_tail;
            /* chain block */
            if (!insn_is_unconditional_branch(last_ir->opcode)) {
                if (rv->is_branch_taken && !last_ir->branch_taken) {
                    last_ir->branch_taken = block->ir_head;
                } else if (!rv->is_branch_taken && !last_ir->branch_untaken) {
...
#if RV32_HAS(EXT_TT)
        /* Execute pending Tensix instructions from FIFO (pushed by sw to INSTRN_BUF_BASE) */
        if (rv->tensix && rv->core_id >= 0 && rv->core_id <= 2) {
            tensix_step(rv->tensix, rv->core_id);
        }
#endif
    }
...
retranslate:
    ...
    /* fetch the next instruction */
    uint32_t insn = rv->io.mem_ifetch(PRIV(rv), rv->PC);
#if RV32_HAS(SYSTEM)
    if (!insn && need_retranslate) {
        need_retranslate = false;
        goto retranslate;
