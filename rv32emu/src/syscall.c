...
static void syscall_gettimeofday(riscv_t *rv)
{
    ...
    if (tv) {
        ...
        memory_write_w(PRIV(rv)->mem, tv + 0, (const uint8_t *) &tv_s.tv_sec);
        memory_write_w(PRIV(rv)->mem, tv + 8, (const uint8_t *) &tv_s.tv_usec);
    }
...
static void syscall_clock_gettime(riscv_t *rv)
{
    ...
    if (tp) {
        ...
        memory_write_w(PRIV(rv)->mem, tp + 0, (const uint8_t *) &tp_s.tv_sec);
        memory_write_w(PRIV(rv)->mem, tp + 8, (const uint8_t *) &tp_s.tv_nsec);
    }
