...
#define IO_HANDLER_IMPL(type, op, RW)                                     \
    static IIF(RW)(                                                       \
        /* W */ void MEMIO(op)(riscv_t * rv, riscv_word_t addr,    \
                               riscv_##type##_t data),                    \
        /* R */ riscv_##type##_t MEMIO(op)(riscv_t * rv,           \
                                           riscv_word_t addr))            \
    {                                                                     \
        IIF(RW)                                                           \
        (memory_##op(PRIV(rv)->mem, addr, (uint8_t *) &data), return memory_##op(PRIV(rv)->mem, addr)); \
    }
...
#if RV32_HAS(EXT_TT)
/* Tensix MMIO read handler: dispatch to tt_cop, fallback to normal memory read */
static riscv_word_t on_mem_read_w(riscv_t *rv, riscv_word_t addr)
{
    uint32_t result;
...
#if RV32_HAS(EXT_TT)
/* Tensix MMIO write handler: dispatch to tt_cop, fallback to normal memory write */
static void on_mem_write_w(riscv_t *rv, riscv_word_t addr, riscv_word_t data)
{
    if (rv->tensix && tensix_mmio_write(rv->tensix, rv->core_id, addr, data))
        return;

    memory_write_w(PRIV(rv)->mem, addr, (uint8_t *) &data);
}
#else
...
riscv_t *rv_new(void *attr, riscv_word_t start, void *tensix, int core_id)
{ 
    riscv_t *rv = calloc(1, sizeof(*rv));
    if (!rv) return NULL;

    rv->data = attr;

    assert(rv_set_pc(rv, start));
...
#if RV32_HAS(EXT_TT)
    /* Set Tensix coprocessor pointer and core ID
     * Note: tensix_init is called in QEMU's start_tensix_node(), not here.
     * This is because rv_new creates a single baby core, while tensix_init
     * initializes the entire Tensix core (5 baby cores).
     */
    rv->core_id = core_id;
    rv->tensix = tensix;
#endif
