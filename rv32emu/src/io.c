...
void memory_read(const memory_t *mem,
                 ...
                 uint32_t size)
{
    uint8_t *src = addr < HIGH_MEM_OFFSET ? mem->mem_base + addr :
        (addr - HIGH_MEM_OFFSET) < mem->local_size ? mem->mem_local + (addr - HIGH_MEM_OFFSET) : mem->mem_high + (addr - HIGH_MEM_OFFSET);
    memcpy(dst, src, size);
}

uint32_t memory_ifetch(memory_t *mem, uint32_t addr)
{
    return *(const uint32_t *) (mem->mem_base + addr);
}

#define MEM_READ_IMPL(size, type)                   \
    type memory_read_##size(memory_t *mem, uint32_t addr)          \
    {                                               \
        uint8_t *src = addr < HIGH_MEM_OFFSET ? mem->mem_base + addr : \
            (addr - HIGH_MEM_OFFSET) < mem->local_size ? mem->mem_local + (addr - HIGH_MEM_OFFSET) : mem->mem_high + (addr - HIGH_MEM_OFFSET); \
        return *(type *) src; \
...
#define MEM_WRITE_IMPL(size, type)                                 \
    void memory_write_##size(memory_t *mem, uint32_t addr, const uint8_t *src)    \
    {                                                              \
        uint8_t *dst = addr < HIGH_MEM_OFFSET ? mem->mem_base + addr : \
            (addr - HIGH_MEM_OFFSET) < mem->local_size ? mem->mem_local + (addr - HIGH_MEM_OFFSET) : mem->mem_high + (addr - HIGH_MEM_OFFSET); \
